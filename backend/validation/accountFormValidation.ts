...
export function requestAccountValidation(request: AccountForm) {
  ...
  if (!passwordFormat.test(request.password)) {
    return {
      success: false,
      error:
...
export function requestChangePasswordValidation(request: string) {
  // if (!request) {
  //   return { success: false, error: "Enter new password" };
  // }
  if (!passwordFormat.test(request)) {
    return {
      success: false,
      error:
        "Password must contain minimum 8 characters, at least one letter and one number",
